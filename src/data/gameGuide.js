...
export const GAME_GUIDE_SECTIONS = [
  ...
  },
  {
    ...
    title: 'Game modes',
    body: [
      '<strong>Frontline Command</strong> (Standard) — Full skirmish vs AI on any map. Pick Recruit, Regular, or Veteran enemy experience. On the battlefield screen choose <strong>Central Command</strong> (single HQ — all unit types from headquarters) or <strong>Forward Bases</strong> (<strong>Large map only</strong> — build an <strong>Infantry Garrison</strong> and depots from HQ; train units at completed structures; expand at <strong>captured sectors</strong>). Both command structures start <strong>each side</strong> with a <strong>radio operator and infantry squad</strong>; Central Command exposes all unit types from HQ while Forward Bases unlocks them through structures. Longer-paced fights: tougher units, slower damage, slower income/production.',
      '<strong>Fortified Line</strong> (Clear Defenses) — Dug-in positions across the map; <em>no HQ, capture points, or sector economy</em>. On the battlefield options screen choose <strong>Assault the Position</strong> or <strong>Command the Garrison</strong> alongside reinforcement size — both sides are auto-deployed (no manual placement). The assault has <strong>15 minutes</strong> to wipe every defender. If any defenders remain when time expires, the attackers lose and receive a mandatory <strong>Full Retreat</strong> order. As <strong>Defender</strong>, hold the prepared trench belt until the deadline or destroy the assault force. A complete force wipe still loses immediately, before later reinforcement waves can restore the line.',
