...
export const GAME_GUIDE_SECTIONS = [
  ...
  },
  {
    ...
    body: [
      ...
      '<strong>Engineer field works:</strong> Select an engineer → <strong>Build sandbags</strong> or <strong>Build bunker</strong> → LMB within ~24 m. The engineer moves to the site and erects the position. <strong>Sandbags</strong> (~11 s) are quick heavy-cover fighting pits. <strong>Bunkers</strong> (~28 s) are sturdier emplacements that <strong>garrison foot troops</strong> (infantry, MG, sniper, medic, engineer) — move a squad onto the completed bunker to enter. Garrisoned units take heavy-cover reduction and can fire out; order a move to exit. A move order during placement or construction cancels the build. In <strong>Base Building</strong> mode engineers can only build bunkers (max 6 per base, shared with HQ bunkers). Available in all modes including Tower Defence HQ Defense (when you have engineers). Esc cancels placement.',
      '<strong>Infantry trenches:</strong> Select infantry, MG, or sniper → <strong>Dig trench</strong> → LMB within ~18 m. The squad digs for ~14 s, then drops into the trench (crouched). Other foot troops can move onto a <strong>friendly or empty enemy trench</strong> to dig in for medium-heavy cover; occupied enemy trenches remain contested and cannot be entered. Order a move to leave or cancel digging. Esc cancels placement.',
      '<strong>Medic field hospital tent:</strong> Select a medic → <strong>Field hospital tent</strong> → LMB within ~18 m. After ~16 s the tent is up; <strong>non-vehicle units</strong> (infantry, MG, sniper, mortar, medics, engineers) within ~12 m slowly heal. A move order during deployment cancels the tent build. Max 4 tents per side. Esc cancels placement.',
      '<strong>Standard bunkers:</strong> Engineer field bunkers in <strong>Classic</strong> Standard, or <strong>Infantry Bunkers</strong> from Base Construction (95 supplies) in Base Building — place near HQ or a sector you hold. Garrison up to <strong>2</strong> foot troops each. Move units onto a friendly or <strong>empty enemy bunker</strong> to enter; occupied enemy bunkers remain unavailable. Order a move away to exit.',
      ...
      'Small-arms tracers only; tanks and artillery use impact VFX. Heavy fire can scar terrain (craters). <strong>Mortar teams</strong> stow their weapon while moving, deploy it when stationary, turn the elevated tube toward their target, and fire from the actual tube muzzle.',
    ],
  },
  {
    id: 'cover',
