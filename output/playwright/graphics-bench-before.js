...
import { buildTerrain, updateUnitTerrainPose } from '/.tmp-graphics-perf-baseline/src/world/Terrain.js';
import { setupRenderer, setupSceneEnvironment, setupLighting } from '/.tmp-graphics-perf-baseline/src/world/SceneSetup.js';
import { preloadUnitTextures, setActiveVehicleTheatre } from '/.tmp-graphics-perf-baseline/src/units/UnitTextures.js';
import { createUnitMesh } from '/.tmp-graphics-perf-baseline/src/units/UnitMeshes.js';
import { createGroundMaterialMaps } from '/.tmp-graphics-perf-baseline/src/world/proceduralTextures.js';
import { MAPS } from '/.tmp-graphics-perf-baseline/src/data/maps.js';
import { FACTIONS } from '/.tmp-graphics-perf-baseline/src/data/factions.js';
...
for(let i=0;i<count;i++){const faction=Object.values(FACTIONS)[i%5],type=types[i%types.length],model=createUnitMesh(type,faction.color,faction.accent,faction.id);model.position.set((i%10-4.5)*7.5,0,(Math.floor(i/10)-4.5)*7.5);if(q.has('look'))model.position.set((i%3-1)*6,0,(Math.floor(i/3)-.5)*8);model.rotation.y=(i%3-1)*.2;updateUnitTerrainPose({mesh:model,def:{type}},map,2);scene.add(model);models.push(model);}
const modelsMs=performance.now()-start;
const camera=new THREE.PerspectiveCamera(46,1280/720,.1,1000);camera.position.set(55,78,96);camera.lookAt(0,0,0);if(q.has('look')){camera.position.set(14,13,25);camera.lookAt(0,1,0);}
