...
import {addExplosionCrater} from '/src/world/TerrainDamage.js';
...
const camera=new THREE.PerspectiveCamera(46,1280/660,.1,1000);camera.position.set(16,28,32);camera.lookAt(0,0,0);if(q.has('close')){camera.position.set(10,9,18);camera.lookAt(10,0,6);}
let frames=0;function render(){renderer.render(scene,camera);if(++frames<15)requestAnimationFrame(render);else{
 ...
 document.querySelector('#status').textContent=JSON.stringify({phase:'after',map:map.id,craters:entries.length,calls:renderer.info.render.calls,triangles:renderer.info.render.triangles,textures:renderer.info.memory.textures,generationMs,heightSignature,centers});
